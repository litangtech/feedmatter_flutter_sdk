...
完整的示例项目请参考 [example](example) 目录。示例项目通过 `feedmatter_flutter_ui` 包展示开箱即用的反馈 UI，业务方无需再复制页面代码。
...
// 1. 应用启动时初始化 SDK（与快速开始相同）
FeedMatterClient.instance.init(config, user, onError: ...);

// 2. 挂载反馈入口
Navigator.push(
  context,
  MaterialPageRoute(builder: (_) => const FeedMatterFeedbackEntry()),
);
```

`FeedMatterFeedbackEntry` 会根据 `ProjectConfig.faqEnabled` 自动展示「常见问题 + 反馈列表」Tab，或在 FAQ 关闭时仅展示反馈首页。

### UI 包目录

```text
packages/feedmatter_flutter_ui/lib/
  feedmatter_flutter_ui.dart
  src/
    pages/
      feedback_entry.dart
...
- 根据项目配置展示 FAQ、反馈、评论、附件、游客权限等开关。
- 常见问题列表、本地搜索、版本缓存、外链回调。
- 反馈列表、关键词搜索、全部反馈 / 我的反馈切换。
- 提交反馈，支持反馈类型选择、字数限制和附件回调。
- 反馈详情展示。
- 楼中楼评论展示、发表评论、回复主评论、加载更多回复。
- 点赞反馈和下拉刷新。

### FeedMatterUiOptions 定制
